...
const SelectTimePage = () => {
  ...
  useEffect(() => {
    console.log("🔍 DEBUG - SelectTimePage State:", {
      isGroupBooking,
      groupMembers,
      selectedServices,
 ...
 const navigateToConfirmation = (appointments) => {
  ...
  // Transform the appointment data to match the expected structure
  const transformedAppointments = appointments.map(appt => ({
    ...
    user: {
      name: appt.memberName, // Use member name for group bookings
      email: user?.email || "",
      phone: user?.phone || "",
      photoURL: ""
    },
  ...
  const confirmationData = {
    ...
    appointmentDetails: transformedAppointments, // Use transformed data
  ...
  console.log("✅ Navigating to confirmation with transformed data:", {
    totalAmount,
