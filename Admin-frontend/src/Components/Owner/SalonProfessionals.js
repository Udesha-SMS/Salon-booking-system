...
// Handle Add/Update Professional - SIMPLE WORKING VERSION
const handleAddOrUpdateProfessional = async () => {
  ...
  try {
    const professionalData = {
      ...
      wellQualified: formData.wellQualified,
    ...
    if (editingProfessional) {
      // UPDATE: Use regular PUT route without files
      url = `http://localhost:5000/api/professionals/${editingProfessional._id}`;
      method = "PUT";
      options = {
        method,
      ...
      console.log("📝 Updating staff (no files):", professionalData);
    } else {
      // CREATE: Use POST with FormData (including files)
      ...
      Object.keys(professionalData).forEach(key => {
        submitData.append(key, professionalData[key]);
      });
      ...
      console.log("➕ Creating new staff with possible files");
    }
